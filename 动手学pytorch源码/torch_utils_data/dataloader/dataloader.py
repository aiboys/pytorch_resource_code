...
class _DatasetKind(object):
    '''
    这个类是用来区分两种dataset类型的，为了后续的一些判别（两种数据类型个支持的sampler或者其他的不一样）
    '''
    Map = 0
    Iterable = 1

# 关于参数的解释： https://blog.csdn.net/g11d111/article/details/81504637
# 关于collate_fn参考 ： https://blog.csdn.net/u012436149/article/details/78545766
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
        ...
        if isinstance(dataset, IterableDataset):
            ...
            # specific workers.
            if shuffle is not False:  # Iterabledataset不支持这三种操作
                raise ValueError(
            ...
            self._dataset_kind = _DatasetKind.Map  # 0

        if sampler is not None and shuffle:  # 给了sampler就不能给shuffle了，两个参数是互斥的
            raise ValueError('sampler option is mutually exclusive with '
                             'shuffle')

        if batch_sampler is not None:   # 自定义batch sampler, 和下面的参数是互斥的
        ...
        if sampler is None:  # give default samplers
            if self._dataset_kind == _DatasetKind.Iterable:
                # See NOTE [ Custom Samplers and IterableDataset ]
                sampler = _InfiniteConstantSampler()  # 可迭代的数据集的sampler是一种虚拟的无限的采样（由于没有了index的保证，并且可能会出现重复的数据，可能会存在有些采样没采到，为了保证尽量的对有限的数据集采样完全）
            else:  # map-style
                if shuffle:
...
##特别关键的dataloaderIter 类，这个类是相当于输出数据的接口
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __next__(self):
        '''
        dataloader获得了迭代器之后，
        for 循环需要调用 __next__() 来获得下一个对象，从而实现遍历。通过 __next__ 方法调用 _next_data() 获取数据
